...
class Puddle:
    ...
    def roll(cls, distribution, p1, p2):
        func = cls.DISTRIBUTIONS.get(distribution)
        return func(p1, p2) if func else 0
    ...
    def add_component(self,
                      ...
                      param2: float):
        try:
            assert str(component_name) not in self._components
        except ValueError:
        ...
        for argname, accepts in {'normalize_mode': ('wrap', 'clamp'),
                                 'distribution': self.DISTRIBUTIONS}.items():
            if args[argname] not in accepts:
                msg = f'"{argname}" must be one of: {", ".join(accepts)}'
...
class Pool:
    ...
    @property
    def puddles(self):
        return [x[0] for x in self._puddles]
